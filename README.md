...
Open the printer, enter a direct PNG, JPEG, WebP, GIF, BMP, TIFF, ICO, or TGA URL, and select **Load**, or select **Browse** to choose a local file. Local files are sent as bounded bytes to the server; their filesystem path never leaves the client. The printer detects the image resolution and aspect ratio, then selects and displays an automatic physical size with a maximum four-block long edge. The **−** and **+** controls resize the print in block units while keeping its aspect ratio; players never need to enter pixel dimensions. Choose a background fill from the **BG** swatches, add the displayed amount of paper and ink, then select **Print**. White remains the default. Hover a swatch for its color name and RGB value; the selected swatch has a gold outline and updates the preview. Large prints consume one sheet of paper per occupied block. A printed item records its immutable variant hash, source dimensions, texture resolution, physical block size, background color, print mode, and title. Printed wall displays use no decorative border.
