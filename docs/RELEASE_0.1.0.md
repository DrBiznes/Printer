...
Status: cartridge, Photobook, background-color, and tooltip implementation is complete for the 0.1.0 release candidate. Fresh-instance testing of the exact final JAR remains mandatory before publication. 0.1.0 requires new worlds; this document is not a release sign-off.
...
| Supplies | One paper per occupied block and one Color Cartridge charge or one Black Ink Cartridge per successful print. A fresh Color Cartridge has three charges. Raw ink sacs are not printer supplies. |
| Output | One Image item; output slot must be empty. Supplies are consumed when the job succeeds. |
| Placement | Supported vertical wall with enough clearance, or ordinary Minecraft item frame. Printed wall displays always use no decorative border. Unprinted Image stacks cannot make wall displays. |
...
- For a player looking at the front panel, paper is on the player's right and output is on the player's left. The README uses this viewpoint; automation tooltips are deferred to future Ponder scenes.
...
The Printer creative tab orders Printer, Black Ink Cartridge, Color Cartridge, then Photobook. Blank Images are excluded from creative/search and rendered crisply in item browsers. No Printer items are duplicated in Functional Blocks. Tooltips provide short translated summaries, remaining charges/photo count or Image title/block size, and a Shift hint. Expanded lines cover the print workflow, ink/background rules, photobook use, and Image placement; automation lives in the README until Ponder.
...
3. **Tooltip presentation:** every item uses translated summary, condition, and behaviour keys where applicable. Printed Images show their title and block size without Shift; Shift reveals brief gray metadata/behaviour lines; the printer workflow also has a gold heading. Image metadata reports background color and never frame data, source URLs, or internal IDs. The common keyboard bridge remains dedicated-server safe.
...
1. **Visual verification:** implemented cartridge/Photobook art, book preview layout, concise tooltips, and blank-Image filtering need maintainer checks in the packaged client, including normal/large GUI scales and JEI.
...
| Tooltips | All five registered items, both Shift states, blank/printed Images, used/depleted cartridges, background color details, and the final Create/AnalogAudio-style formatting; no frame data is shown. |
...
| Release packaging | Candidate version is 0.1.0; the MIT license and credits ship under `META-INF`. Record the candidate checksum and [bug-report path](https://github.com/DrBiznes/Printer/issues). Do not publish until exact-artifact smoke gates pass. |

Full Ponder scenes, Create scenes, CC:Tweaked, version ports, and configurable ink economy are deferred until after 0.1.0. Background color and tooltip presentation are in scope for 0.1.0. Reopening scope requires updating this contract and its verification matrix before cutting a release.

## Upload review

The 2026-09-13 [upload review](UPLOAD_REVIEW_0.1.0.md) records authorization checks, byte/memory/time limits, the upload preflight fix, test evidence, and the distinction between automated readiness and pending client/server smoke tests. The network protocol is 0.1.0 with no old-peer negotiation.
